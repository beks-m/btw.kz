...
class PostList extends Component {
  ...
  getDayName(date) {
    var string = moment(date, 'DD.MM.YYYY').calendar(null, {
      sameDay: 'Добавлено сегодня (примеры)',
      lastDay: 'Вчера',
      lastWeek: 'dddd',
      sameElse: 'DD/MM/YYYY'
    });
                ...
                ?
                  this.props.postList.map((item, idx) => {
                    if (idx < this.state.postCountLimit) {
                      return <PostListItem key={idx} authorizedUser={this.props.authorizedUser}  history={this.props.history} post={item} />
                    } else {
                      return null;
                    }
                  })
